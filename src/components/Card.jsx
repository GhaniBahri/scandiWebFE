...
function Card({product}) {
  return (
    <>
      ...
      hover:scale-105 hover:shadow-[0_0_8px_2px_rgba(0,0,0,0.2)] overflow-hidden transition-all duration-150 ease-in-out group ${product.inStock? '' : ''}`}>
        ...
        <NavLink to={`/products/${product.id}`} className={` text-white rounded-full p-2 bg-primary absolute right-4 bottom-20 transition-opacity ease-out duration-150 opacity-0 ${product.inStock? 'group-hover:opacity-100': ''}`}>
